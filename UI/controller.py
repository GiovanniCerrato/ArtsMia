...
class Controller:
    ...
    def handleCompConnessa(self,e):
        ...
        if txtIdOggetto == "":
            self._view.txt_result.controls.clear()
            self._view.txt_result.controls.append(ft.Text("Attenzione, inserire un valore nel campo id", color = "red" , weight = "bold"))
            self._view.update_page()
            return
        ...
        try:
            ...
            self._view.txt_result.controls.append(
                ft.Text("Attenzione, inserire un valore numerico nel campo id", color="red", weight="bold"))
            self._view.update_page()
            return

        if not self._model.hasNode(idOggetto):
            self._view.txt_result.controls.clear()
            self._view.txt_result.controls.append(ft.Text("Attenzione, l'id inserito non è presente nel grafo", color="red", weight="bold"))
            self._view.update_page()
            return

        sizeCompConn = self._model.getInfoCompConnessa(idOggetto)
        self._view.txt_result.controls.clear()
        self._view.txt_result.controls.append(ft.Text(f"La componente connessa contenente l'oggetto con id {idOggetto} è composta da {sizeCompConn} nodi."))
    ...
    def handleCerca(self,e):
        ...
        if lun is None:
            self._view.txt_result.controls.clear()
            self._view.txt_result.controls.append(ft.Text(f"Attenzione, selezionare un valore di lunghezza fra le scelte proposte", color="red", weight="bold"))
            self._view.update_page()
            return
